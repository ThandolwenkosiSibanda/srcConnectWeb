...
import {  ADMIN_CASH_ADVANCES_QUERY, ADMIN_LOANS_QUERY, ADMIN_PAYMENTS_QUERY } from '../../gql/Query';
...
<div style={{display: 'flex', flexDirection: 'row', justifyContent: 'space-between', width: '100%'}}>
  ...
  <Form.Group className="mb-3" controlId="">
        ...
        <tr >
            ...
            <td>{data?.admin_payments?.length}</td>
...
<Table striped bordered hover>
      ...
      <tbody>
{data?.admin_payments?.filter(promotion => promotion?.moneyMember?.name?.toLowerCase().indexOf(nameFilter?.toLowerCase()) > -1)
.filter(promotion => promotion?.moneyMember?.surname?.toLowerCase().indexOf(surnameFilter?.toLowerCase()) > -1)
.filter(promotion => promotion?.type?.toLowerCase().indexOf(typeFilter?.toLowerCase()) > -1).map((member, index)=><tr key ={index}>
