...
import {  ADMIN_CASH_ADVANCES_QUERY, ADMIN_LOANS_QUERY } from '../../gql/Query';
...
const CashAdvancePage = () => {
    ...
    const handleImageChange = (e) => {
      ...
      const {data, loading, error} = useQuery(ADMIN_CASH_ADVANCES_QUERY, {
...
<div style={{display: 'flex', flexDirection: 'row', justifyContent: 'space-between', width: '100%'}}>
  ...
  <Form.Group className="mb-3" controlId="">
        ...
        <tr >
            ...
            <td>{data?.admin_cashAdvances?.length}</td>
            <td>{data?.admin_cashAdvances?.filter((member)=> member.status === "Approved").length}</td>
            <td>{data?.admin_cashAdvances?.filter((member)=> member.status === "Rejected").length}</td>
            <td>{data?.admin_cashAdvances?.filter((member)=> member.status === "Pending").length}</td>
          </tr>
...
<Table striped bordered hover>
      ...
      <tbody>
{data?.admin_cashAdvances?.filter(promotion => promotion?.moneyMember?.name?.toLowerCase().indexOf(nameFilter?.toLowerCase()) > -1)
...
.filter(promotion => promotion.status.toLowerCase().indexOf(statusFilter?.toLowerCase()) > -1).map((member, index)=><tr key ={index}>
          ...
          <td>
        <Link to={`/cashadvances/${member?._id}`} className="btn btn-primary">View History</Link>
        </td>
