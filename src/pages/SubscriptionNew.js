...
const SubscriptionsNew = () => {
  ...
  const [form, setForm] = useState({
    ...
    platform: { label: "Cash", value: "cash" },
  ...
  const handleSave = async () => {
    try {
      ...
      // Check for recent similar submissions within 1 minute
      const { data: recentSubs, error: checkError } = await supabase
        .from("subscriptions")
        .select("id, transaction_date")
        .eq("policy_id", policy_id)
        .eq("client_id", client_id)
        .gte("transaction_date", oneMinuteAgo);

      if (checkError) throw checkError;

      if (recentSubs?.length > 0) {
        throw new Error(
          "You just submitted this subscription. Please wait a moment before submitting again."
        );
      }
  ...
  return (
    <>
      ...
      <PageTitle name={"New Subscription"} />
